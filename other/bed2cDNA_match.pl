...
#			$all_tag[-1]=~s/[SH](\d+)//g;
			...
			print "$temp[0]\tcotton\tcDNA_match\t$exon_start\t$temp[2]\t$score\t$temp[5]\t.\tID=$hash{$temp[3]}->[0].path1;Name=$hash{$temp[3]}->[0];Target=$hash{$temp[3]}->[0] $start $end;Gap=$gap_string\n";
		}else{
			if(!defined $reads{$temp[3]}->[1]){
...
#			$all_tag[-1]=~s/[SH](\d+)//g;
			...
			$gap_string=join " ",@all_tag;
			print "$temp[0]\tcotton\tcDNA_match\t$exon_start\t$temp[2]\t$score\t$temp[5]\t.\tID=$hash{$temp[3]}->[0].path1;Name=$hash{$temp[3]}->[0];Target=$hash{$temp[3]}->[0] $end $start;Gap=$gap_string\n";
		}
		$reads{$temp[3]}->[1]=$end;
	}
}
