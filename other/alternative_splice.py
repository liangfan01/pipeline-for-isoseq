...
class Cluster():
    ...
    def _varyedge(self, coors, args):
        deledges = []
        for i in range(1, len(coors)):
            if coors[i] - coors[i-1] <= args.asvaryedge: # filter 3 bases
                deledges += coors[i-1:i+1]
        # if deledges:
        #     print deledges, '#####1'
        #     print [edge for edge in coors if edge not in deledges], '####2'
        #     sys.exit()
        return [edge for edge in coors if edge not in deledges]
        ...
        for geneID in self.alternativeSplice:
            ...
            for i ,j in combinations(self.alternativeSplice[geneID], 2):
                ...
                if coorintronis[0][2] != coorintronjs[0][2]:
                    ...
                    for k in cluster:
                        s, e = eval(k)
                        if Coordinate().coorCmp((s, e), rangeis, 1)[1] in [3, 4] and \
                            Coordinate().coorCmp((s, e), rangejs, 1)[1] in [3, 4]:
                            coors = sum(cluster[k], [])
                            coors = self._varyedge(sorted([k for k in coors if coors.count(k) == 1 and k not in ['+', '-']]), args)#modify
                            if coors:
                                if strand == "-":
                                    coors.reverse()
...
demo
    ...
    parser.add_argument('-ave','--asvaryedge',metavar = 'int',type = float,default = 3,
        help = 'filter alternative splice event if edge different length in [0, $ASVARYEDGE].')
